...
void simple_net(const int batch, const int n_iter) {
	...
	auto conv_weights_md = memory::desc({conv_weights_tz}, memory::data_type::f32, memory::format::any);
	...
	/* execute */
	while (n_iter_left) {
		std::cout << "TRAINING : " << n_iter - n_iter_left << "/" << n_iter << std::endl;
		/* forward */
		...
		// some user defined func
		//update_diff_dst(net_diff_dst.data(), net_output)

		stream(stream::kind::eager).submit(net_bwd).wait();
		/* update weights and bias using diff weights and bias*/
		auto net_diff_weights = conv_user_diff_weights_memory.get_data_handle();
		auto net_diff_bias = conv_diff_bias_memory.get_data_handle();
		/* ...user updates weights and bias using diff weights and bias...*/
		// some user defined func
		//update_weights(conv_weights.data(), conv_bias.data(), net_diff_weights, net_diff_bias);
...
int main(int argc, char **argv) {
	...
	const int batch = 1536;

	try {
		simple_net(batch, n_iter);
